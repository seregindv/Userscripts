...
function RunShowMore() {
    ...
    if (!autoCheckBox)
        autoCheckBox = document.getElementById("autoCheckBox");
    if (autoCheckBox.checked) {
        const oneDay = GetElementByAttribute(document, "time", "data-short", "1 dy");
        if (oneDay !== null) {
            ShowLinkSummary(true);
            return;
        }
    }
...
function ShowLinkSummary(openVideo) {
    imageSet = new Set();
    ...
    if (span != null) {
        ...
        for (var key in linkCounts) {
            const linkCount = linkCounts[key];
        ...
        for (var i = 0; i < linkCountArray.length; i++) {
            const linkCount = linkCountArray[i];
...
function AddStop() {
    ...
    auto.id = "autoCheckBox";
    auto.name = "autoCheckBox";
    auto.checked = true;
    div.appendChild(auto);

    var label = document.createElement("label");
    label.setAttribute("for", "autoCheckBox");
